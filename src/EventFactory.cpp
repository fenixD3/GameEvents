...
std::shared_ptr<IEvent> EventFactory::CreateEvent(const std::vector<std::string>& event_tokens)
{
    ...
    else
    {
        auto event = std::visit(details::overloaded{
            [&event_tokens](const EventFactory::MapCreationTag&) -> std::shared_ptr<IEvent>
                {
            ...
            [&event_tokens](const EventFactory::SpawnCreatureTag&) -> std::shared_ptr<IEvent>
                {
                    try
                    {
            ...
            [&event_tokens](const EventFactory::MarchTag&) -> std::shared_ptr<IEvent>
                {
                    try
                    {
            ...
            [&event_tokens](const EventFactory::WaitTag&) -> std::shared_ptr<IEvent>
                {
                    try
                    {
                        return std::make_shared<WaitEvent>(boost::lexical_cast<int>(event_tokens[1]));
                    }
                    catch (std::exception& ex)
                    {
                        throw std::invalid_argument("Invalid input for SPAWN "s + ex.what());
            ...
            [](const EventFactory::FinishTag&) -> std::shared_ptr<IEvent>
            {
